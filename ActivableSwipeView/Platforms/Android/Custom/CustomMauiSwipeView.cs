...
{
    internal class CustomMauiSwipeView : ContentViewGroup
    {
        ...
        public CustomMauiSwipeView(Context context) : base(context)
        {
            _context = context;
        ...
        bool ProcessTouchMove(APointF point)
        {
            ...
            if (!_isOpen)
            {
                ...
                _swipeDirection = ActivableSwipeView.Helper.SwipeDirectionHelper.GetSwipeDirection(new Point(_initialPoint.X, _initialPoint.Y), new Point(point.X, point.Y));
        ...
        void UpdateSwipeItemViewLayout(ISwipeItemView swipeItemView)
        {
            ...
            ActivableSwipeView.Helper.ViewHandlerExtensions.LayoutVirtualView(handler, 0, 0, (int)swipeItemSize.Width, (int)swipeItemSize.Height);

            if (swipeItemView?.Handler != null)
                ActivableSwipeView.Helper.ElementHandlerExtensions.ToPlatform(swipeItemView.Handler).InvalidateMeasure(swipeItemView);
        }
        ...
        static bool GetIsVisible(ISwipeItem swipeItem)
        {
            if (swipeItem is IView view)
                return view.Visibility == Microsoft.Maui.Visibility.Visible;
            else if (swipeItem is ISwipeItemMenuItem menuItem)
                return menuItem.Visibility == Microsoft.Maui.Visibility.Visible;
